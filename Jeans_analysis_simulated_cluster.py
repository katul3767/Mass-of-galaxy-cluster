...
# getting the profile
bins, dbins, err, mean = velocity_dispersion_profile(r_proj.value, los_v.value, 10, method="biweight")
bins1, dbins1, err1, mean1=velocity_dispersion_profile(r_proj.value, los_v.value, 10, method="std")
bins2, dbins2, err2, mean2 =velocity_dispersion_profile(r_proj.value, los_v.value, 10, method="rms")
bins3, dbins3, err3, mean3 =velocity_dispersion_profile(r_proj.value, los_v.value, 10, method="gapper")
...
def get_sigma_all(dndV):
    ...
    sigma_all=np.array([integrate.quad(sigma, r_i, 1.90, args=(r_i))[0] for r_i in r_mid])
    return sigma_all
...
plt.plot(r_mid,radial_vd,'o',linestyle= '-')
plt.xlabel("$\mathrm{r~[Mpc]}$")
plt.ylabel("$\mathrm{\sigma_{radial}~[kms^{-1}]}$")
plt.show()

#finding the slop of the density as well as velocity profiler_mid
dln_nu=r_mid /nu_all_diff *(np.gradient(nu_all_diff,r_mid))
dln_sigma=r_mid/radial_vd*(np.gradient(radial_vd,r_mid))
dln_nu,dln_sigma

# mass profile
vel_disp=1040*10**3 # Km to meters
